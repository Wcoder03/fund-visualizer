...
export default function HoldingSummary({ holdings, snapshotsByFundCode = {}, lastUpdatedAt }: HoldingSummaryProps) {
  ...
  const stats: StatItem[] = [
    ...
    { label: '持有收益', value: totalProfit !== null ? <ProfitLossValue value={totalProfit} type="money" className="text-[22px] font-bold" /> : emptyVal, icon: iconProfit, accent: true, color: 'from-rose-500 to-rose-600' },
    { label: '持有收益率', value: totalRate !== null ? <ProfitLossValue value={totalRate} type="rate" className="text-[22px] font-bold" /> : emptyVal, icon: iconRate, accent: true, color: 'from-pink-500 to-pink-600' },
    { label: '今日收益', value: dailyProfit !== null ? <ProfitLossValue value={dailyProfit} type="money" className="text-[22px] font-bold" /> : emptyVal, icon: iconDaily, accent: true, color: 'from-amber-500 to-orange-500' },
    { label: '今日收益率', value: dailyRate !== null ? <ProfitLossValue value={dailyRate} type="rate" className="text-[22px] font-bold" /> : emptyVal, icon: iconDailyRate, accent: true, color: 'from-yellow-500 to-amber-500' },
    ...
    <section className="ui-card overflow-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4">
        {stats.map((stat, i) => (
          <div
            key={stat.label}
            className={`group relative flex flex-col justify-center px-6 py-5 transition-colors hover:bg-slate-50/50 ${
              i % 4 !== 3 ? 'border-r border-slate-100/80' : ''
            } ${i < 4 ? 'border-b border-slate-100/80' : ''}`}
          >
            <div className="flex items-center gap-2">
              <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br ${stat.color} text-white shadow-sm`}>
                {stat.icon}
              </span>
              <span className="text-[13px] font-medium text-slate-400">{stat.label}</span>
            </div>
            <div className="mt-3 pl-[36px] text-[22px] font-bold tracking-tight text-slate-900 tabular-nums" style={{ letterSpacing: '-0.02em' }}>
              {stat.value}
            </div>
          </div>
        ))}
