...
export default function NavStatusBadge({ status, marketType }: NavStatusBadgeProps) {
  ...
  return <span className={`inline-flex h-[22px] items-center whitespace-nowrap rounded-full px-2.5 text-[12px] font-medium ring-1 ${className}`}>{navStatusLabel(status, marketType)}</span>;
}
